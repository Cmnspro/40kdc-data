...
<div
	class="flex items-stretch gap-1.5 rounded border px-2 py-1.5 transition-colors {selected
		...
		: 'bg-panel-surface border-panel-border hover:bg-panel-hover'}"
>
	...
	<button class="flex min-w-0 flex-1 flex-col text-left" onclick={onselect}>
		<div class="flex items-center gap-1.5">
			...
			{#if unit.modelCount > 1}<span class="text-text-muted shrink-0 tabular-nums text-xs"
				...
				>{/if}
			{#if unit.isWarlord}<span class="rounded bg-amber-900/50 px-1 py-0.5 text-xs text-amber-200">WL</span>{/if}
			{#if unit.attachedToKey}<span class="rounded bg-teal-900/50 px-1 py-0.5 text-xs text-teal-200">attached</span>{/if}
			{#if leadingCount > 0}<span class="rounded bg-teal-900/50 px-1 py-0.5 text-xs text-teal-200"
		...
		{#if enhancement}
			<span class="truncate text-xs text-accent" title={`Enhancement: ${enhancement}`}>{enhancement}</span>
		{/if}
		{#if summary}
			<span class="text-text-dim truncate text-xs" title={summary}>{summary}</span>
		{/if}
	</button>

	<div class="flex shrink-0 items-center gap-2">
		<span class="text-text tabular-nums text-sm font-medium">{points}</span>
