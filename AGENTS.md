...
tools/              TypeScript package @alpaca-software/40kdc-data:
                      ...
                      src/mfm/          MFM contract and ingestion:
                        dump-catalog.schema.json
                                            validation schema for dump.catalog.json
                        mfm-source-map.schema.json
                                            validation schema for mappings/*.mapping.json
                        dump.catalog.json    hand-reviewed table/field/relation semantics
                        dump.schema.json     generated, committed public source-shape schema
                        dump.generated.ts    generated typed table/relation contract
                        mappings/            one provenance map per schemas/core schema
                        gen-dump-schema.ts    contract generator/checker/report command
                        loader.ts            shared runtime table/relation access
examples/           Demo web apps consuming the package (Svelte 5 + Vite +
                      Cloudflare). See examples/AGENTS.md for their shared
...
- MFM structure metadata—table/field names, JSON types, requiredness/nullability,
  row counts, identities/relations, IP classifications, source maps, and
  generated TypeScript row types—is permitted and intentionally committed/published.
- NEVER commit `_private/dump.json`, dump row values, localized GW rules/lore,
  or artwork/asset URLs. `_private/dump.schema.json` is an ignored local mirror;
  the committed public schema is `tools/src/mfm/dump.schema.json`.
...
- Changed the MFM catalog, mappings, generator, or private snapshot? From
  `tools/`, run `npm run mfm:contract -- --write`, then `--check`, then
  `--report`; finish at the repository root with `just preflight`. The snapshot
  commands require ignored `_private/dump.json`; CI and contributors without it
  exercise the public contract through fabricated `test/mfm-*.test.ts` fixtures.
...
9. If the entity's data files live under per-faction dirs
   ...
   (no per-record id).
10. Every new `schemas/core/*.schema.json` also requires
    `tools/src/mfm/mappings/<schema-basename>.mapping.json`. Cover every
    instance pointer, including repository-authored, runtime-input, not-in-MFM,
    and unresolved fields—not only fields currently populated from MFM.
11. Run `npm test && npm run validate`.
...
- **GW MFM dump**: `_private/dump.json` is the ignored raw Munitorum Field
  ...
  has ~130 tables covering datasheets, loadouts, enhancements, detachment rules,
  stratagems, and missions. It is **authoritative** for the live game and
  supersedes army-assist → convert-faction as the upstream for mechanical data.
  Run `npx tsx tools/src/ingest-mfm.ts <subcommand>` (coverage / dispositions /
...
## MFM contract workflow

`_private/dump.json` is the authoritative raw MFM export and MUST remain
gitignored. Its structure is intentionally public and committed:

- `tools/src/mfm/dump-catalog.schema.json` and
  `tools/src/mfm/mfm-source-map.schema.json` — validation schemas for the two
  public contracts;
- `tools/src/mfm/dump.catalog.json` — hand-reviewed semantics, identities, and
  relations;
- `tools/src/mfm/dump.schema.json` — generated public JSON Schema for the
  observed dump shape;
- `tools/src/mfm/dump.generated.ts` — generated table rows and verified
  relation types;
- `tools/src/mfm/mappings/*.mapping.json` — one source/provenance map for every
  `schemas/core/*.schema.json`;
- `tools/src/mfm/gen-dump-schema.ts` — the generator, checker, and coverage
  reporter behind `npm run mfm:contract`;
- `tools/src/mfm/loader.ts` — the shared runtime table/relation API.

These files replace session-local row interfaces, guessed joins, and duplicate
walkers. Before adding MFM plumbing, update the catalog or affected mapping,
regenerate, and consume generated row types plus verified `MFM_RELATIONS`
through `loader.ts`; never add a parallel manual MFM model.

For a new dump snapshot:

1. Replace only `_private/dump.json`.
2. From `tools/`, run `npm run mfm:contract -- --check` to expose structural
   drift without writing.
3. Review every new/changed field and update `dump.catalog.json` plus every
   affected core-schema mapping; do not infer relation meaning from field names.
4. Run `npm run mfm:contract -- --write`, then `--check`, then `--report`.
5. Run the focused MFM tests and root `just preflight`.

The public contract may contain names, types, nullability, counts, identities,
relations, and provenance. It MUST NOT contain dump row values, localized GW
prose, or artwork/asset URLs. BSData 11e is a warning-only backstop; it never
